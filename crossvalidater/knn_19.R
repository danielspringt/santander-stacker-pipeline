...
info <- list()
model.name <- paste0("knn_19_","kernel_",kern,"_k",n.k)
...
for(i in 1:length(flds)){
  ...
  cv.train <- xg.train[train.index,] ### anpassen
  cv.test <- xg.train[test.index,]   ### anpassen
  ...
  #------------- Training test------------#
  Pred.test <- predict(train.con, xg.test) ### anpassen
  #------------- Training test------------#
