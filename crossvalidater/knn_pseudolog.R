...
info <- list()
model.name <- paste0("knn_pseudolog10_","kernel_",kern,"_k",n.k)
...
for(i in 1:length(flds)){
  ...
  cv.train <- train.10.log[train.index,] ### anpassen
  cv.test <- train.10.log[test.index,]   ### anpassen
  ...
  #------------- Training test------------#
  Pred.test <- predict(train.con, test.10.log) ### anpassen
  #------------- Training test------------#
