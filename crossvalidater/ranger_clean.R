...
info <- list()
model.name <- paste0("ranger_clean_","mtry",ranger.mtry,"_ntree",ranger.ntree)
...
for(i in 1:length(flds)){
  ...
  cv.train <- train.clean[train.index,] ### anpassen
  cv.test <- train.clean[test.index,]   ### anpassen
  ...
  #------------- Training test------------#
  Pred.test <- predict(forest.cv, test.clean) ### anpassen
