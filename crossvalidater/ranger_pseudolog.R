...
info <- list()
model.name <- paste0("ranger_pseudolog_","mtry",ranger.mtry,"_ntree",ranger.ntree)
...
for(i in 1:length(flds)){
  ...
  cv.train <- train.log[train.index,] ### anpassen
  cv.test <- train.log[test.index,]   ### anpassen
  ...
  #------------- Training test------------#
  Pred.test <- predict(forest.cv, test.log) ### anpassen
