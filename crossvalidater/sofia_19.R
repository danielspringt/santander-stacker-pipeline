...
info <- list()
model.name <- paste0("ranger_19_","mtry",ranger.mtry,"_ntree",ranger.ntree)
...
for(i in 1:length(flds)){
  ...
  #------------- Training cv--------------#
  model.logreg <- sofia(TARGET ~ ., 
                        data=cv.train[,-target], 
                        learner_type="logreg-pegasos",
                        loop_type = loop,
                        verbose=TRUE
                        )
  
  p <- predict(model.logreg, cv.test, prediction_type = "logistic",probabili)
  ...
  #------------- Training test------------#
  Pred.test <- predict(forest.cv, test.raw) ### anpassen
